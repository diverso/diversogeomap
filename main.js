...
function loadMapFromURL(url, redirectedFromHTML)
{
    ...
    httpRequest.onload = function (e) {
        ...
        if (httpRequest.status != 200) {
            displayErrorMessage(dashcode.getLocalizedString("Error while retrieving the map information."));
        ...
        if (parsedDoc) {
            ...
            displayErrorMessage(dashcode.getLocalizedString("The source document could not be processed."));
        }
...
function load()
{
    ...
    if (titleEl) {
        if(titleEl.innerHTML.length && titleEl.innerHTML != "Map title shows here  ") {
            hasCustomTitle = true;
        }
        else {
            titleEl.innerHTML = "";
    ...
    if (!attributes.googleMapsKey || attributes.googleMapsKey.length == 0) {
        displayErrorMessage(dashcode.getLocalizedString("Please specify a Maps API Key in the Widget Attributes."), true);
    }
    else {
        ...
        // else show an error message
            ...
            displayErrorMessage(dashcode.getLocalizedString("Error loading the Maps API."));
        }
    }
}
